...
void Skybox::constructBox()
{
	...
	glBindTexture(GL_TEXTURE_2D, stars.id); // Use texture we want
...
Skybox::Skybox(Vec3 mn, Vec3 mx)
{
	...
	{
	stars = Texture(STARS_TEXTURE, NIGHT_BOX_TOP, SKYBOX_SIDES, SKYBOX_SIDES);
	side = Texture(SIDE_TEXTURE, NIGHT_BOX_SIDE, SKYBOX_SIDES, SKYBOX_SIDES);
	bottom = Texture(BOTTOM_TEXTURE, NIGHT_BOX_BOTTOM, 2, 2);
	}
	constructBox();
}
